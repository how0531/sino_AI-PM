...
> - **確認關卡簡化為一次(實測成功)**:原本開單有兩層確認(Agent 指示問一次「是否寫入 Planner」+ 工具「執行之前先詢問」再彈一次),重複。把「建立工作」工具的「執行之前先詢問」關掉(=否),只留 Agent 指示層那次(含詳細內容)的確認。現在:指派任務 → 列出詳情問一次 → 回「確認」→ 直接建立。對外送的 Teams/Outlook 工具仍保留確認關卡(不可逆,較謹慎)。
