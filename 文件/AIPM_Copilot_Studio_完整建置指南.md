...
> - **KPI 缺口(Gap)分析(實測成功)**:KPI 目標值寫進 KPI Agent 指示(底稿見 `素材/KPI目標.xlsx`,可維護後改)——完成率目標 80%、逾期率上限 10%、成員負載上限 5 張/人、里程碑準時率 90%。實測「算 KPI 跟目標比缺口」→ 讀 Planner 5 張任務,算出完成率 0% vs 目標 80%(缺口 −80% 未達標)、逾期率與負載達標,逐項標達標/未達標+資料品質提醒。(截圖:成品/截圖_KPI對照目標算缺口.png)
> - **確認關卡簡化為一次(實測成功)**:原本開單有兩層確認(Agent 指示問一次「是否寫入 Planner」+ 工具「執行之前先詢問」再彈一次),重複。把「建立工作」工具的「執行之前先詢問」關掉(=否),只留 Agent 指示層那次(含詳細內容)的確認。現在:指派任務 → 列出詳情問一次 → 回「確認」→ 直接建立。**全部五個寫入/外送工具(建立工作、更新工作、會議記錄+時程的 Teams/Outlook)都已統一關掉工具層確認,只留 Agent 指示層一次確認**(實測 Teams 送訊息:列草稿問一次 → 確認 → 直接送達)。確認的把關改由 Agent 指示負責(指示明訂寫入/外送前一律先列內容給 PM 確認)。
